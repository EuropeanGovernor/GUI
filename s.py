...
def add2_todayselect(name,p):##将菜品添加到today_select
    ...
    ns=':'.join(ns)
    cursor_obj.execute('INSERT INTO history(date,nutrition) VALUES(?,?)',(today,ns))
    con.commit()
###  输入的name是一个菜名字符串,p是int份数
# add2_todayselect('鸡蛋',4)
# add2_todayselect('豆浆',2)
def week_assess():##周食谱评估
    ...
    criterion=[weight*7,weight*7,weight*35,5000*7,30*7,100*7,1000*7,12.5*7,15*7,30*7,weight*280]
###正常人一天：蛋白质（1g*weight），脂肪（1g*weight），碳水化合物（5g*weight）
    # 维a(5000国际单位)，维b（30mg）,维c（100mg）
    ...
    num=1
    if x[0]<0.75:
        advice+=str(num)+'.'+'您上周蛋白质摄入不足，若不及时调整，可能会导致水肿，生长迟缓，身体消瘦，贫血，发育不良等'+'\n'
        num+=1
    if x[0]>1.25:
        advice+=str(num)+'.'+'您上周蛋白质摄入超标，若不及时调整，可能会加重肾脏和肝的负担，也导致骨质疏松，痛风，还可能引起泌尿系统结石和便秘'+'\n'
        num+=1
    if x[1]<0.75:
        advice+=str(num)+'.'+'您上周脂肪摄入不足，若不及时调整，可能会导致营养不良，肠胃功能下降皮肤受损，生殖障碍，代谢能力降低，维生素D缺乏'+'\n'
        num+=1
    if x[1]>1.25:
        advice+=str(num)+'.'+'您上周脂肪摄入超标，若不及时调整，可能会增加肠胃负担，导致身体肥胖，高血脂高血压，脂肪肝等'+'\n'
        num+=1
    if x[2]<0.75:
        advice+=str(num)+'.'+'您上周碳水化合物摄入不足，若不及时调整，可能会造成营养不良，低血糖，平时无精打采，易头晕心慌'+'\n'
        num+=1
    if x[2]>1.25:
        advice+=str(num)+'.'+'您上周碳水化合物摄入超标，若不及时调整，可能会变胖'+'\n'
        num+=1
    if x[3]<0.75:
        advice+=str(num)+'.'+'您上周维a摄入不足，若不及时调整，可能会导致皮肤干燥，夜盲症，发育迟缓，肌肉乏力，食欲减退等'+'\n'
        num+=1
    if x[3]>1.25:
        advice+=str(num)+'.'+'您上周维a摄入超标，若不及时调整，可能会维a慢性中毒，导致关节酸痛，脱发，头痛，嗜睡，视力模糊等'+'\n'
        num+=1
    if x[4]<0.75:
        advice+=str(num)+'.'+'您上周维b摄入不足，若不及时调整，可能会导致口腔溃疡，脚气病，牙周炎，皮肤瘙痒，神经衰弱等'+'\n'
        num+=1
    if x[4]>1.25:
        advice+=str(num)+'.'+'您上周维b摄入超标，若不及时调整，可能会导致维b慢性中毒，头晕眼花，腹泻，心律失常，发热呕吐，影响发育'+'\n'
        num+=1
    if x[5]<0.75:
        advice+=str(num)+'.'+'您上周维c摄入不足，若不及时调整，可能会导致皮肤黏膜出血，骨骼病变，疲倦乏力'+'\n'
        num+=1
    if x[5]>1.25:
        advice+=str(num)+'.'+'您上周摄入超标，若不及时调整，可能会导致尿结石，产生骨骼疾病。'+'\n'
        num+=1
    if x[6]<0.75:
        advice+=str(num)+'.'+'您上周钙摄入不足，若不及时调整，可能会导致骨质疏松，关节疼痛，抽筋，烦躁，水肿，佝偻病，发育迟缓等'+'\n'
        num+=1
    if x[6]>1.25:
        advice+=str(num)+'.'+'您上周钙摄入超标，若不及时调整，可能会导致肠胃损伤，软组织钙化，泌尿系统结石'+'\n'
        num+=1
    if x[7]<0.75:
        advice+=str(num)+'.'+'您上周铁摄入不足，若不及时调整，可能会导致贫血，四肢无力，精神不振，昏迷甚至休克'+'\n'
        num+=1
    if x[7]>1.25:
        advice+=str(num)+'.'+'您上周铁摄入超标，若不及时调整，可能会导致肺脾淋巴系统铁沉积，还影响心脑血管，慢性铁中毒，甚至食道癌大肠癌，'+'\n'
        num+=1
    if x[8]<0.75:
        advice+=str(num)+'.'+'您上周锌摄入不足，若不及时调整，可能会发育不良，厌食挑食，免疫力低下，乏力'+'\n'
        num+=1
    if x[8]>1.25:
        advice+=str(num)+'.'+'您上周锌摄入超标，若不及时调整，可能会导致锌中毒，消化道糜烂出血，呼吸道炎症，低铜血症，神经功能障碍'+'\n'
        num+=1
    if x[9]<0.75:
        advice+=str(num)+'.'+'您上周膳食纤维摄入不足，若不及时调整，可能会引起痔疮，便秘，身体肥胖，糖尿病，诱发结肠癌和心脑血管疾病'+'\n'
        num+=1
    if x[9]>1.25:
        advice+=str(num)+'.'+'您上周膳食纤维摄入超标，若不及时调整，可能会引起肠胃不适，影响无机盐的吸收'+'\n'
        num+=1
    if x[10]<0.75:
        advice+=str(num)+'.'+'您上周热量摄入不足，若不及时调整，可能会导致各种系统功能衰退，呆滞，记忆力减退，神志不清，浑身乏力'+'\n'
        num+=1
    if x[10]>1.25:
        advice+=str(num)+'.'+'您上周摄入超标，若不及时调整，可能会导致肥胖，进而诱发乳腺癌大肠癌'+'\n'
        num+=1
    return x,advice

###  x是一个有11个浮点数的列表，例如[0.71,0.88,0.98,0.78,0.45,0.66,0.77,0.88,0.99,0.85]
